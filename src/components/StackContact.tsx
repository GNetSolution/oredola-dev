  ...
  return (
    <>
      <section className="relative mx-auto max-w- px-6 md:px-8 py-20 border-t overflow-hidden" style={{ borderColor: t.border, background: t.bg }}>
        <div className="relative grid grid-cols-1 md:grid-cols-[0.85fr_0.6fr_1fr] gap-8 items-start z-10">

          {/* LEFT - ORBIT SEPARATE */}
          <div className="relative h- flex items-center justify-center">
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
              ...
              <motion.div animate={{ rotate: 360 }} transition={{ duration: 24, repeat: Infinity, ease: "linear" }} className="absolute h- w-">
                {[STACK[0], STACK[2], STACK[4]].map((s, idx) => {
                  ...
                  return (
                    <div key={s.name} className="absolute left-1/2 top-1/2" style={{ transform: `translate(-50%, -50%) rotate(${angle}deg) translateY(-180px)` }}>
                      <motion.div animate={{ rotate: -360 }} transition={{ duration: 24, repeat: Infinity, ease: "linear" }} className="h-9 w-9 rounded-full border backdrop-blur-xl flex items-center justify-center text- font-black" style={{ background: isLight? "#0a0a0a" : "#fff", color: isLight? "#fff" : "#000", borderColor: t.border }}>{s.icon}</motion.div>
                    </div>
                  );
                })}
              </motion.div>
              <motion.div animate={{ rotate: -360 }} transition={{ duration: 18, repeat: Infinity, ease: "linear" }} className="absolute h- w-">
                {[STACK[1], STACK[3], STACK[5]].map((s, idx) => {
                  const angle = idx * 120;
                  return (
                    <div key={s.name} className="absolute left-1/2 top-1/2" style={{ transform: `translate(-50%, -50%) rotate(${angle}deg) translateY(-130px)` }}>
                      <motion.div animate={{ rotate: 360 }} transition={{ duration: 18, repeat: Infinity, ease: "linear" }} className="h-7 w-7 rounded-full border backdrop-blur-xl flex items-center justify-center text- font-bold" style={{ background: isLight? "rgba(0,0,0,0.06)" : "rgba(255,255,255,0.08)", color: t.fg, borderColor: t.border }}>{s.icon}</motion.div>
                    </div>
                  );
                })}
              </motion.div>
              <div className="absolute h-16 w-16 rounded-full border backdrop-blur-xl flex items-center justify-center font-mono text- font-bold" style={{ background: t.card, borderColor: t.border, color: t.fg }}>STACK</div>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="mt-4 text- leading-[1.6]" style={{ color: isLight? "rgba(0,0,0,0.55)" : "rgba(255,255,255,0.45)" }}>Not tutorial stack. This is what I use to ship AcademiaBase, RankEngine, StudentPay.</p>
            <div className="mt-6 rounded- border p-4 font-mono text- leading-6 backdrop-blur-xl" style={{ borderColor: t.border, background: t.card, color: isLight? "rgba(0,0,0,0.55)" : "rgba(255,255,255,0.4)" }}>
              <div>✓ 8 products shipped</div><div>✓ 1000+ builders mentored</div><div>✓ {time || "Loading..."}</div>
            </div>
          </div>
